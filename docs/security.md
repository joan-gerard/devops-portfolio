...
- **Database**: `lib/db.ts` validates `DATABASE_URL` lazily and uses Neon's serverless HTTP driver (`@neondatabase/serverless`). Use parameterized queries (e.g. `sql\`...\${userInput}\`` tagged templates) when you add queries to avoid SQL injection.
...
- **Frontend**: Public note HTML is generated with TipTap's `generateHTML` (strict schema from `lib/tipTapExtensions.ts`, including table nodes). Note detail pages (`components/notes/NoteDetail.tsx`) pass that HTML through DOMPurify (`dompurify`) in `renderRichContentHtmlWithToc` on the client before the final `dangerouslySetInnerHTML`; the initial server HTML for the note body is the same schema-generated string (DOMPurify and code highlighting run in the browser because that helper uses `window`). The about page (`components/public/about/AboutPageContent.tsx`) uses the same schema-generated HTML without sanitization, as all content is first-party from the single admin. External links use `rel="noopener noreferrer"` where appropriate.
