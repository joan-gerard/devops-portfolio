...
- **Admin auth**: Admin sign-in uses NextAuth with a credentials provider; the password is stored as a bcrypt hash in `ADMIN_PASSWORD_HASH`. Misconfiguration and bcrypt errors are handled with a generic “Sign-in is temporarily unavailable” message so no server details are exposed. See [Authentication](authentication.md) for full details.
