...
- Current:
  - `components/public/roadmap/RoadmapSidePanel.tsx`
...
- Current:
  ...
  - public and admin nodes/components are split into different directories.
- Opportunity:
  - keep `roadmapFlowMapper.ts` as-is, but align module ownership by moving/placing the public/admin node components alongside the mapper feature (same top-level folder).
...
- Current: roadmap has `components/public/roadmap/index.ts` and `components/roadmap/index.ts`, but when reorganizing into deeper folders (nodes/panels/public/admin) it becomes easy to miss exports.
- Opportunity:
