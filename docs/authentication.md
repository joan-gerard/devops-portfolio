...
- After auth, the layout renders **`AdminShell`** (`components/dashboard/AdminShell.tsx`), a client component that lays out the fixed **`AdminSidebar`**, **`AdminHeader`**, and main content. The header includes a control to collapse or expand the sidebar (shared `--admin-sidebar-offset`); the sidebar uses the `inert` attribute when collapsed so focus does not move into hidden navigation. Open/closed preference is stored in **`localStorage`** (`devops-portfolio/admin-sidebar-open`) and mirrored in a **`devops-portfolio_admin_sidebar` cookie** (`0` / `1`) so the server can pass **`initialSidebarOpen`** and **`hadCookie`**. Initial UI state is computed with **`computeInitialSidebarOpen`** in `lib/adminSidebarStorage.ts` (no cookie: prefer `localStorage` when set; cookie present: prefer the server). A layout effect only writes to storage to match that state—no `setState` in effects. Toggles update both stores; other tabs pick up `localStorage` via the `storage` event.
...
7. **SignOutButton**: Uses `type="button"` and explicit hover handlers (no form) so behavior is clear and predictable.
